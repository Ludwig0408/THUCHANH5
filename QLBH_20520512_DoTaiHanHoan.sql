...
(
	...
	NGHD SMALLDATETIME,
	MAKH CHAR(4) CONSTRAINT FK_HOADON_KHACHHANG FOREIGN KEY REFERENCES KHACHHANG(MAKH),
	MANV CHAR(4) CONSTRAINT FK_HOADON_NHANVIEN FOREIGN KEY REFERENCES NHANVIEN(MANV),
	TRIGIA MONEY
)
...
--4. Sửa kiểu dữ liệu của thuộc tính GHICHU trong quan hệ SANPHAM thành VARCHAR1(100).
ALTER TABLE SANPHAM
...
--5. In ra các số hóa đơn, trị giá hóa đơn bán ra trong ngày 1/1/2007 và ngày 2/1/2007.
SET DATEFORMAT DMY
SELECT SOHD, TRIGIA
FROM HOADON
